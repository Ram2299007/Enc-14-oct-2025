...
public class DocumentDownloadPreviewAdapter extends RecyclerView.Adapter<DocumentDownloadPreviewAdapter.DocumentDownloadViewHolder> {
    
    private Context context;
    private List<Uri> documentUris;
    
    public DocumentDownloadPreviewAdapter(Context context, List<Uri> documentUris) {
    ...
    @NonNull
    @Override
    public DocumentDownloadViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_document_download_preview, parent, false);
        return new DocumentDownloadViewHolder(view);
    }
    
    @Override
    public void onBindViewHolder(@NonNull DocumentDownloadViewHolder holder, int position) {
        Uri documentUri = documentUris.get(position);
        
        // Get file name
        String fileName = getFileName(documentUri);
        holder.docName.setText(fileName);
    ...
    private void setDocumentIcon(ImageView iconView, String type) {
        switch (type.toLowerCase()) {
            ...
            case "img":
                ...
                iconView.setColorFilter(context.getResources().getColor(R.color.gray3));
                break;
    ...
    public static class DocumentDownloadViewHolder extends RecyclerView.ViewHolder {
        TextView docName;
        TextView size;
        ImageView documentIcon;
        
        public DocumentDownloadViewHolder(@NonNull View itemView) {
            super(itemView);
